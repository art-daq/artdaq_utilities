		...
		for (auto& metric : metricData_)
		{
			if (readyToSend_(metric.first) || forceSend)
			{
				...
				else if (!metric.second.empty())
				{
					...
					{
						sendMetric_(data.Name + data.GetSuffix(MetricMode::Accumulate), data.Value, data.Unit, data.Type, to_system_clock(lastSendTime_[data.Name]));
					}
					if ((data.Mode & MetricMode::RunningSum) != MetricMode::None)
					{
						sendMetric_(data.Name + data.GetSuffix(MetricMode::RunningSum), metricRegistry_[metric.first].Value, data.Unit, data.Type, to_system_clock(lastSendTime_[data.Name]));
					}
					if ((data.Mode & MetricMode::Average) != MetricMode::None)
					{
						...
						sendMetric_(data.Name + data.GetSuffix(MetricMode::Average), average, data.Unit, to_system_clock(lastSendTime_[data.Name]));
					}
					if ((data.Mode & MetricMode::Rate) != MetricMode::None)
					{
						...
						sendMetric_(data.Name + data.GetSuffix(MetricMode::Rate), rate, data.Unit + "/s", to_system_clock(lastSendTime_[data.Name]));
					}
					if ((data.Mode & MetricMode::Minimum) != MetricMode::None)
					{
						sendMetric_(data.Name + data.GetSuffix(MetricMode::Minimum), data.Min, data.Unit, data.Type, to_system_clock(lastSendTime_[data.Name]));
					}
					if ((data.Mode & MetricMode::Maximum) != MetricMode::None)
					{
						sendMetric_(data.Name + data.GetSuffix(MetricMode::Maximum), data.Max, data.Unit, data.Type, to_system_clock(lastSendTime_[data.Name]));
					}
...
private:
	...
	void sendZero_(MetricData data)
	{
		if (sendZeros_)
		{
			...
			if ((data.Mode & MetricMode::LastPoint) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::LastPoint), zero, data.Unit, data.Type, std::chrono::system_clock::now());
			}
			if ((data.Mode & MetricMode::Accumulate) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::Accumulate), zero, data.Unit, data.Type, std::chrono::system_clock::now());
			}
			if ((data.Mode & MetricMode::RunningSum) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::RunningSum), zero, data.Unit, data.Type, std::chrono::system_clock::now());
			}
			if ((data.Mode & MetricMode::Average) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::Average), 0.0, data.Unit, std::chrono::system_clock::now());
			}
			if ((data.Mode & MetricMode::Rate) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::Rate), 0.0, data.Unit + "/s", std::chrono::system_clock::now());
			}
			if ((data.Mode & MetricMode::Minimum) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::Minimum), zero, data.Unit, data.Type, std::chrono::system_clock::now());
			}
			if ((data.Mode & MetricMode::Maximum) != MetricMode::None)
			{
				sendMetric_(data.Name + data.GetSuffix(MetricMode::Maximum), zero, data.Unit, data.Type, std::chrono::system_clock::now());
			}
		}
	}
