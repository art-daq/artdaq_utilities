...
public:
   ...
   * "uniquify" (Default: false): If true, will replace %UID% with the PID of the current process, or append _%UID% to
   ...
   */
	explicit FileMetric(fhicl::ParameterSet const& config, std::string const& app_name)
	    ...
	    , use_logroot_env_(pset.get<bool>("use_logroot_env", false))
	    ...
	    , stopped_(true)
	{
		...
		if (use_logroot_env_)
		{
			...
			if (logRootString != NULL)
			{
				std::string logRoot = std::string(logRootString);
				outputFile_ = logRoot + "/" + outputFile_;
			}
			else
			{
				TLOG(TLVL_WARNING) << "ARTDAQ_LOG_ROOT was not defined! Defaulting to /tmp!";
